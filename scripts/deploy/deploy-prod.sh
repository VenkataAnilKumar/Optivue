...
if [[ -z "$PYTHON_BIN" ]]; then
	...
	if [[ -z "$POWERSHELL_BIN" ]]; then
		echo "[deploy-prod] Could not find a Python executable with pytest installed or a PowerShell fallback."
		exit 1
	fi
fi

echo "[deploy-prod] Running mandatory checks"
...
echo "[deploy-prod] This script does not auto-deploy to protect prod safety policies."
echo "[deploy-prod] Execute reviewed deployment manually:"
echo "  cd src/infra && npm run deploy"
